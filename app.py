...
from database import load_data, save_data, DEFAULT_COLORS, DATA_FILE, BACKUP_DIR

st.set_page_config(
    page_title="UGAR brewpub - Csaplista", page_icon="🍺", layout="wide"
)

# Adatok betöltése a biztonságos tárolóból
data, load_msg = load_data()

if load_msg:
    if "Sikerült" in load_msg:
        st.success(load_msg)
    else:
        st.info(load_msg)
...
def get_all_active_system_beers(data):
    beers = set()
    ...
    for r in data.get("raktar", []):
        beers.add(r)

    for k in data.get("kuka", []):
        beers.add(k)

    return sorted(list(beers))

# LETISZTÍTOTT, EGYESÍTETT CSS CSOMAG
bg_tap = "#1E88E5" if st.session_state["active_tab"] == "🚰 Csapok" else "#222"
bg_trash = "#E53935" if st.session_state["active_tab"] == "🗑️ Üres Hordók" else "#222"
bg_mgm = "#43A047" if st.session_state["active_tab"] == "⚙️ Menedzsment & Raktár" else "#222"
...
st.html(
    ...
    <style>
        ...
        div.stButton > button[key='nav_tap'] {{
            background-color: {bg_tap} !important;
        ...
        div.stButton > button[key='nav_trash'] {{
            background-color: {bg_trash} !important;
            color: white !important;
        ...
        div.stButton > button[key='nav_mgm'] {{
            background-color: {bg_mgm} !important;
            color: white !important;
...
with nav_col3:
    if st.button("🗑️ Üres Hordók", key="nav_trash", use_container_width=True):
        st.session_state["active_tab"] = "🗑️ Üres Hordók"
        st.rerun()

with nav_col4:
    if st.button("⚙️ Menedzsment & Raktár", key="nav_mgm", use_container_width=True):
    ...
    for c in data["csapok"]:
        ...
        with c3:
            if c["kovetkezo"]:
                ...
                arrow = " <span style='color:#888; font-weight:bold;'>➜</span> "
                st.markdown(
...
# 2. TAB: ÜRES HORDÓK (KUKA)
    ...
    with col_k_undo:
        if data.get("kuka_history") and len(data["kuka_history"]) > 0:
            if st.button("↩️ Legutóbbi elszállítás visszavonása", key="undo_kuka_btn"):
    ...
    with col_k_save:
        if data.get("kuka_history") and len(data["kuka_history"]) > 0:
            if st.button(
                ...
                data["kuka_history"] = []
                save_data(data)
                st.success("Elszállítások rögzítve! A visszavonási lehetőség lezárva.")
                st.rerun()
        ...
        for beer_name, count in sorted(kuka_counts.items()):
            ...
            with k_col0:
                if st.button(
                    ...
                    tort_name = (
                        beer_name if "(tört)" in beer_name else f"{beer_name} (tört)"
                    )
                    data["raktar"].append(tort_name)

                    base_name = beer_name.replace(" (tört)", "").strip()
                    if base_name in data["szinek"] and tort_name not in data["szinek"]:
                        data["szinek"][tort_name] = data["szinek"][base_name]

                    save_data(data)
                    st.success(f"'{tort_name}' visszaküldve a raktárba tört hordóként!")
                    st.rerun()
                ...
                if st.button(
                    ...
                    for _ in range(take_count):
                        if beer_name in data["kuka"]:
                            data["kuka"].remove(beer_name)
                    save_data(data)
                    st.success(f"{take_count} db '{beer_name}' elszállítva!")
                    st.rerun()
...
# 3. TAB: MENEDZSMENT & RAKTÁR
    ...
    with m1:
        ...
        with st.container(border=True):
            if not st.session_state["confirm_wash"]:
                ...
                with wc1:
                    if st.button("✅ Igen", key="save_wash_btn", use_container_width=True):
                        ...
                        st.rerun()
                with wc2:
                    if st.button("❌ Mégse", key="cancel_wash_btn", use_container_width=True):
                        st.session_state["confirm_wash"] = False
                        st.rerun()
    ...
    with m2:
        ...
        with st.container(border=True):
            if not st.session_state["confirm_co2"]:
                ...
                with cc1:
                    if st.button("✅ Igen", key="save_co2_btn", use_container_width=True):
                        ...
                        st.rerun()
                with cc2:
                    if st.button("❌ Mégse", key="cancel_co2_btn", use_container_width=True):
                        st.session_state["confirm_co2"] = False
                        st.rerun()
    ...
    with col_r1:
        ...
        if data.get("raktar"):
            ...
            for beer_name, count in sorted(raktar_counts.items()):
                ...
                with r_col3:
                    if st.button(
                        ...
                        if selected_tap_str == "— Válassz csapot —":
                            st.error("Kérlek válassz ki egy csapot a legördülő menüből!")
                        else:
                            target_c_id = int(
                ...
                with r_col4:
                    if st.session_state["confirm_delete_beer"] == beer_name:
                        ...
                        if st.button(
                            ...
                            st.session_state["confirm_delete_beer"] = beer_name
                            st.rerun()
    ...
    with col_r2:
        with st.container(border=True):
            ...
            add_type = st.segmented_control(
                "Típus kiválasztása:",
                ["Meglévő sörök", "Tört hordó hozzáadása", "Vadonatúj sör"],
                default="Meglévő sörök",
                key="add_type_segmented",
            )

            rendszerben_levo_sorok = get_all_active_system_beers(data)

            if add_type == "Meglévő sörök":
                tisztitott_sorok = sorted(
                    list(set([s for s in rendszerben_levo_sorok if "(tört)" not in s]))
                )

                if tisztitott_sorok:
                    valasztott_meglevo = st.selectbox(
                        "Válassz a rendszerben lévő sörökből:",
                        tisztitott_sorok,
                    ...
                    if st.button(
                        ...
                        save_data(data)
                        st.success(
                            f"{db_szam} db '{valasztott_meglevo}' hozzáadva a raktárhoz!"
                        )
                        st.rerun()
                else:
                    st.warning("Nincs egyetlen elérhető sör sem a rendszerben!")
                    ...
                    list(
                        set(
                            [
                                ...
                                for s in rendszerben_levo_sorok
                            ]
                        )
                    )
                )
                ...
                if minden_alap_sor:
                    ...
                    if st.button(
                        ...
                        save_data(data)
                        st.success(
                            f"{db_szam_tort} db '{tort_teljes_nev}' hozzáadva a raktárhoz!"
                        )
                        st.rerun()
                else:
                    st.warning("Nincs elérhető sör a tört hordó készítéséhez!")
                ...
                if st.button(
                    "Új Sör Mentése", key="add_new_beer_btn", use_container_width=True
                ):
                    if uj_sor_nev.strip():
                        ...
                        save_data(data)
                        st.success(f"{db_szam_uj} db '{s_nev}' hozzáadva a raktárhoz!")
                        st.rerun()
    ...
    for c in data["csapok"]:
        if c["kovetkezo"]:
            ...
            with v_col2:
                kov_badges = [format_beer_badge(k) for k in c["kovetkezo"]]
                arrow_html = " <span style='color: #888; font-weight: bold;'>➜</span> "
                st.markdown(arrow_html.join(kov_badges), unsafe_allow_html=True)

            with v_col3:
                if st.button(f"↩️ Visszahívás", key=f"recall_btn_{c['id']}"):
                    recalled = c["kovetkezo"].pop()
                    data["raktar"].append(recalled)
                    save_data(data)
                    st.success(f"'{recalled}' visszakerült a raktárba a #{c['id']} csapról!")
                    st.rerun()
